 ...
 * boot. Split by domain: 11 HR (P3-03) + 11 Marketing (P3-04) + 1 Sales (WhatsApp
 * sales agent). Every manifest is validated by `validateManifest` on seed and by
 * `workflow-templates.catalog.spec.ts`.
 */
export const FIRST_PARTY_WORKFLOW_TEMPLATES: readonly WorkflowTemplateManifest[] =
  [...HR_WORKFLOW_TEMPLATES, ...MARKETING_WORKFLOW_TEMPLATES, ...SALES_WORKFLOW_TEMPLATES];
