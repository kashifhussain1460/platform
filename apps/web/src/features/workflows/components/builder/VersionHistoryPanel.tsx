...
const VERSION_STATUS_CLS: Record<WorkflowVersionStatus, string> = {
  DRAFT: 'bg-white/[0.06] text-wf-ink-2',
  PUBLISHED: 'bg-status-succeeded/10 text-status-succeeded',
  DEPRECATED: 'bg-white/[0.04] text-wf-ink-3',
  ARCHIVED: 'bg-white/[0.04] text-wf-ink-3',
};

const linkBtn =
  'text-xs font-medium text-wf-ink-2 transition-colors hover:text-wf-ink focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-wf-focus rounded';
    ...
    return (
      <div className="space-y-2" aria-busy="true">
        {Array.from({ length: 3 }).map((_, i) => (
          <div key={i} className="h-14 animate-pulse rounded-xl border border-wf-hairline bg-void-card" />
        ))}
      </div>
    );
  }
  ...
  return (
    <>
          ...
          return (
            <li
              ...
              className={`rounded-xl border p-3 ${
                isActive ? 'border-violet-secondary/40 bg-violet/[0.06]' : 'border-wf-hairline bg-void-card'
              }`}
            >
              <div className="flex items-center gap-2">
                <span className="font-display text-sm font-semibold text-wf-ink">v{v.version}</span>
                <span
                ...
                {isActive ? (
                  <span className="rounded-full bg-violet/15 px-2 py-0.5 text-[10px] font-medium text-violet-secondary">
                    Live
                  </span>
                ) : null}
              ...
              {v.changeNote ? <p className="mt-1.5 text-xs text-wf-ink-2">{v.changeNote}</p> : null}
      ...
      >
        ...
        {restore.isError ? (
          <p className="mt-2 text-sm text-status-failed">{restore.error.message}</p>
        ) : null}
        <div className="mt-4 flex items-center justify-end gap-2">
          <button
            type="button"
            onClick={() => setRestoreVersion(null)}
            className="rounded-lg border border-wf-hairline px-4 py-1.5 text-sm font-medium text-wf-ink-2 hover:text-wf-ink"
          >
            Cancel
          </button>
          <button
